	...
	const series: SeriesView[] = [...grouped.entries()]
		.map(([recurrenceId, rows]) => {
			...
			return {
				...
				reason: first.reason,
				weekdayLabel: `${WEEKDAY_NAMES_SR[weekday]}`,
				...
				total: sorted.length,
				upcoming: sorted.filter((r) => new Date(r.starts_at).getTime() > now)
					.length,
		...
		.sort(
			(a, b) =>
				b.upcoming - a.upcoming || a.courtName.localeCompare(b.courtName),
		);
		...
		<main className="mx-auto w-full max-w-6xl flex-1 px-4 pb-16">
			<div className="pt-5">
				...
				<p className="mt-1 max-w-2xl text-sm text-muted-foreground">
					Nedeljni termini koji se automatski ponavljaju. Kreirajte jednom za
					ceo period — svi termini se posle brišu ili produžavaju jednim klikom,
					bez prolaska kroz kalendar.
				</p>
			</div>
